...
class PlannerService:
    ...
    async def suggest(
        ...
        *,
        ...
        if self.notifier is not None:
            await self.notifier.send_plan(draft)
        return draft

    async def regenerate(self, plan_id: str, feedback: str | None = None) -> MealPlanDraft:
    ...
    async def accept(self, plan_id: str) -> AcceptResult:
        ...
        human_conflicts = [
            item for item in existing if item.get("entryType") == "dinner" and not item.get("createdByMealiePlanner")
        ]
        if human_conflicts:
            raise RuntimeError("Refusing to overwrite human-created Mealie dinner entries")

        for entry_id in self.store.list_created_entries(plan_id):
        ...
        self.store.mark_accepted(plan_id, created)
